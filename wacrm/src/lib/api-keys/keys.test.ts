...
describe('hashApiKey', () => {
  it('is deterministic', () => {
    expect(hashApiKey('vbuild_live_abc')).toBe(hashApiKey('vbuild_live_abc'));
  });

  it('differs for different inputs', () => {
    expect(hashApiKey('vbuild_live_abc')).not.toBe(hashApiKey('vbuild_live_abd'));
  });
});
...
describe('timingSafeHexEqual', () => {
  it('is true for identical digests', () => {
    const h = hashApiKey('vbuild_live_xyz');
