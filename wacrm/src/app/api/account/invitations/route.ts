...
export async function POST(request: Request) {
  try {
    ...
    // Check that the account hasn't reached its max_users limit before
    // creating a new invitation. Count current members (profile rows)
    // against the plan's max_users cap.
    const memberLimit = await checkPlanLimit(ctx.accountId, 'users')
