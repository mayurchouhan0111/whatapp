...
export async function POST(
  ...
  if (status === 'active') {
    // Resolve the account_id for this flow and check plan gates.
    const { data: flowRow } = await admin
      ...
      .maybeSingle()
    if (flowRow) {
      const flowsEnabled = await checkFeatureGate(flowRow.account_id, 'flows')
      if (!flowsEnabled) {
        return NextResponse.json(
          { error: 'Chatbot Flows are not available on your current plan. Please upgrade to enable this feature.' },
          { status: 403 },
        )
      }
      const activeLimit = await checkPlanLimit(flowRow.account_id, 'active_flows')
      if (!activeLimit.allowed) {
        return NextResponse.json(
          { error: activeLimit.message },
          { status: 403 },
        )
      }
    }
