...
async function upsertTemplateRow(
  ...
  // NOTE: The upsert conflict target now uses `account_id` instead of `user_id`.
  // A migration (015_message_templates_account_unique.sql) updates the
  // unique index to (`account_id`, `name`, `language`). This prevents
  // teammates from shadowing each other's templates with the same name.
  return supabase
