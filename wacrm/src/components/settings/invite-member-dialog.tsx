...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Vbuild CRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    const accountName = result?.accountName ?? 'our Vbuild CRM account';
    const message = `Join ${accountName} on Vbuild CRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
